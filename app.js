...
onAuthStateChanged(auth, async (user) => {
    ...
    if (ud.exists()) {
        ...
        if (isMonitorOnly) applyMonitorMode();
...
window.openChangePassModal = () => { document.getElementById('changePassModal').classList.add('show'); };
...
window.navTo = async (sec) => {
    if (!isAdmin) {
        const permMap = { pos: 'pos', products: 'products', invoices: 'invoices', reports: 'reports', users: 'users', settings: 'settings' };
...
function renderProductsGrid() {
    ...
    try {
        ...
        filtered.forEach(p => {
            ...
            const code = p.code || '';
            html += `<div class="product-card" onclick="addToCart('${p.id}')"><div class="cat-img ${catData.class}"><i class="fas ${catData.icon}"></i><span class="cat-label">${p.category || 'أخرى'}</span></div><div class="prod-body"><div class="code">${code}</div><div class="name">${name}</div><div class="price">${price} <span>ج.م</span></div></div></div>`;
        });
        box.innerHTML = html;
...
window.addToCart = (pid) => {
    ...
    else cart.push({ id: pid, name: p.name, price: p.price, qty: 1, note: '' });
    renderCart();
};
...
window.saveNote = () => {
    ...
    if (item) { item.note = note; renderCart(); }
    closeNoteModal();
};
...
window.checkout = async () => {
    ...
    try {
        ...
        const invRef = await addDoc(collection(db, "invoices"), {
            invoiceNumber: invoiceNumber, items: items, subTotal: calc.subTotal,
            discountPercent: calc.discountPercent, discountAmount: calc.discountAmount,
            deliveryFee: calc.deliveryFee, total: calc.finalTotal, paidAmount: calc.paidAmount,
            change: calc.remaining, paymentMethod: cartPaymentMethod,
            itemCount: cart.reduce((s, i) => s + i.qty, 0),
            cashierId: currentUser.uid, cashierName: userData?.fullName || currentUser.email,
...
function renderEditInvoiceItems() {
    ...
    currentEditItems.forEach((item, idx) => {
        ...
        html += `<tr><td><strong>${item.name}</strong></td><td>${item.price} ج.م</td><td><div class="cart-qty" style="margin:0;"><button onclick="changeEditItemQty(${idx}, -1)">−</button><span>${item.qty}</span><button onclick="changeEditItemQty(${idx}, 1)">+</button></div></td><td style="font-weight:800;">${item.price * item.qty} ج.م</td><td style="font-size:12px;color:#888;">${note ? '<i class="fas fa-sticky-note" style="color:var(--warning);"></i> ' + note : '-'}</td><td><button class="btn btn-red" style="padding:4px 10px;font-size:12px;" onclick="removeEditItem(${idx})" title="حذف"><i class="fas fa-trash"></i></button></td></tr>`;
    });
...
window.changeEditItemQty = (idx, delta) => {
    ...
    if (currentEditItems[idx].qty <= 0) {
        if (confirm('الكمية صارت صفر. هل تريد حذف المنتج من الفاتورة؟')) currentEditItems.splice(idx, 1);
        else currentEditItems[idx].qty = 1;
    }
    renderEditInvoiceItems();
    recalcEditInvoice();
};
...
window.closeAddProdToInvModal = () => { document.getElementById('addProdToInvModal').classList.remove('show'); };
...
window.filterAddProdSearch = () => {
    ...
    const filtered = allProducts.filter(p => (p.name || '').toLowerCase().includes(q) || (p.code || '').toLowerCase().includes(q));
    if (filtered.length === 0) {
    ...
    let html = '';
    filtered.forEach(p => {
        html += `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px;border-bottom:1px solid #f0f0f0;cursor:pointer;" onclick="addProductToCurrentEdit('${p.id}')" onmouseover="this.style.background='#f8f9fa'" onmouseout="this.style.background='white'"><div><strong>${p.name}</strong><div style="font-size:12px;color:#888;">${p.code || ''} - ${p.price} ج.م</div></div><button class="btn btn-main" style="padding:4px 10px;font-size:12px;"><i class="fas fa-plus"></i> إضافة</button></div>`;
    });
    box.innerHTML = html;
};
...
window.addProductToCurrentEdit = (pid) => {
    ...
    if (existing) existing.qty++;
    else currentEditItems.push({ name: p.name, price: p.price, qty: 1, total: p.price, note: '' });
    renderEditInvoiceItems();
    recalcEditInvoice();
...
window.saveInvoiceEdit = async () => {
    ...
    const historyEntry = {
        editedAt: new Date().toISOString(), editedBy: currentUser.uid, editedByName: userData?.fullName || currentUser.email,
        reason: reason, note: noteText, oldTotal: currentEditInvoice.total || 0, newTotal: total,
        oldItemsCount: (currentEditInvoice.items || []).reduce((s, i) => s + (i.qty || 1), 0), newItemsCount: itemCount
    };
    ...
    try {
        await updateDoc(doc(db, "invoices", currentEditInvoice.id), {
            items: currentEditItems, subTotal: subTotal, discountPercent: discountPercent,
            discountAmount: discountAmount, deliveryFee: deliveryFee, total: total,
            itemCount: itemCount, editHistory: editHistory, lastEditedAt: serverTimestamp(), lastEditedBy: currentUser.uid
        });
...
window.printInvoiceSingle = async () => {
    ...
    if (invId) {
        ...
        allInvs.forEach(d => { setDoc(doc(db, "invoices", d.id), { printed: true, printedAt: serverTimestamp() }, { merge: true }); });
    }
    window.print();
};
...
window.printInvoiceFromTable = async (id) => {
    ...
    if (!d.exists()) return;
    const inv = d.data();
    const calc = { subTotal: inv.subTotal || inv.total || 0, discountPercent: inv.discountPercent || 0, discountAmount: inv.discountAmount || 0, deliveryFee: inv.deliveryFee || 0, finalTotal: inv.total || 0, paidAmount: inv.paidAmount || 0, remaining: inv.change || 0 };
...
window.editProduct = async (id) => {
    ...
    try {
        ...
        updateEditProdCatSelect();
        if (productCategories.includes(p.category)) catSelect.value = p.category;
        else if (p.category) { productCategories.push(p.category); updateEditProdCatSelect(); catSelect.value = p.category; }
        else catSelect.value = 'أخرى';
        document.getElementById('editNewCatRow').classList.add('hidden');
        document.getElementById('editNewCatInput').value = '';
        document.getElementById('editProductModal').classList.add('show');
    } catch (e) { alert('خطأ في تحميل بيانات المنتج: ' + e.message); }
};
...
window.saveProductEdit = async () => {
    ...
    if (!name || !price) { alert('املأ البيانات'); return; }
    try {
        await setDoc(doc(db, "products", editingProductId), { name, price, category: cat, updatedAt: serverTimestamp(), updatedBy: currentUser.uid }, { merge: true });
...
async function loadInvoicesTable() {
    ...
    try {
        ...
        snap.forEach(d => {
            ...
            if (isAdmin || checkPerm('invoices_view')) viewBtn = `<button class="btn btn-sec" onclick="viewInvoice('${d.id}')" style="padding:5px 10px;" title="عرض"><i class="fas fa-eye"></i></button>`;
            if (isAdmin || checkPerm('invoices_edit')) editBtn = ` <button class="btn btn-warn" onclick="openEditInvoiceModal('${d.id}')" style="padding:5px 10px;" title="تعديل"><i class="fas fa-edit"></i></button>`;
            if (isAdmin || checkPerm('invoices_print')) printBtn = ` <button class="btn btn-blu" onclick="printInvoiceFromTable('${d.id}')" style="padding:5px 10px;" title="طباعة"><i class="fas fa-print"></i></button>`;
            if (isAdmin || checkPerm('invoices_delete')) deleteBtn = ` <button class="btn btn-red" onclick="deleteInvoice('${d.id}')" style="padding:5px 10px;" title="حذف"><i class="fas fa-trash"></i></button>`;
...
window.printDayReport = () => { if (!requirePerm('reports_print', 'طباعة تقرير اليوم')) return; window.print(); };
window.printReports = () => { if (!requirePerm('reports_print', 'طباعة التقارير')) return; window.print(); };
...
async function loadReports() {
    try {
        ...
        snap.forEach(d => {
            ...
            if (inRange) {
                filteredTotal += total; filteredCount++;
...
window.togglePerm = (el) => {
    ...
    console.log('Permission toggled:', cb.value, cb.checked);
};
...
window.updateUser = async () => {
    ...
    console.log('Saving permissions:', perms);
    const updates = { fullName: name, role: role, status: status, permissions: perms, updatedAt: serverTimestamp(), updatedBy: currentUser.uid };
    if (role !== 'admin') {
    ...
    try {
        ...
        snap.forEach(d => {
            ...
            if (isAdmin || checkPerm('users_edit') || checkPerm('users_delete')) {
                ...
                if (isAdmin || checkPerm('users_edit')) acts += `<button class="btn btn-sec" onclick="openEditUserModal('${d.id}')" title="تعديل"><i class="fas fa-edit"></i></button>`;
                ...
                if (isAdmin || checkPerm('users_delete')) acts += `<button class="btn btn-red" onclick="deleteUser('${d.id}')" title="حذف"><i class="fas fa-trash"></i></button>`;
                acts += `</div>`;
...
window.holdInvoice = async () => {
    ...
    try {
        await addDoc(collection(db, "pending_invoices"), {
            ...
            cashierName: userData?.fullName || currentUser.email, createdAt: serverTimestamp()
        });
...
window.closePendingModal = () => { document.getElementById('pendingModal').classList.remove('show'); };
...
window.loadPendingInvoices = async () => {
    ...
    try {
        ...
        else q = query(collection(db, "pending_invoices"), where("cashierId", "==", currentUser.uid));
        const snap = await getDocs(q);
        ...
        docs.forEach(p => {
            ...
            html += `<tr><td>${count}</td><td>${date}</td><td>${itemCount} عنصر</td><td style="font-weight:800;color:var(--primary);">${p.total || 0} ج.م</td><td>${p.cashierName || '-'}</td><td><div style="display:flex;gap:4px;flex-wrap:wrap;"><button class="btn btn-grn" onclick="resumePendingInvoice('${d.id}')" style="padding:5px 10px;"><i class="fas fa-play"></i> استعادة</button><button class="btn btn-blu" onclick="printPendingInvoice('${d.id}')" style="padding:5px 10px;"><i class="fas fa-print"></i> طباعة</button><button class="btn btn-red" onclick="deletePendingInvoice('${d.id}')" style="padding:5px 10px;"><i class="fas fa-trash"></i> حذف</button></div></td></tr>`;
        });
        tbody.innerHTML = html;
    } catch (e) { tbody.innerHTML = '<tr><td colspan="6" class="empty">خطأ</td></tr>'; }
};
...
window.printPendingInvoice = async (id) => {
    ...
    if (!d.exists()) return;
    const p = d.data();
    const calc = { subTotal: p.subTotal || 0, discountPercent: p.discountPercent || 0, discountAmount: p.discountAmount || 0, deliveryFee: p.deliveryFee || 0, finalTotal: p.total || 0, paidAmount: p.paidAmount || 0, remaining: p.remaining || 0 };
...
window.getSelectedInvoices = () => Array.from(document.querySelectorAll('.inv-select:checked')).map(cb => cb.value);

window.printSelectedInvoices = async () => {
    const ids = getSelectedInvoices();
    if (ids.length === 0) { alert('اختر فواتير أولاً'); return; }
    if (!requirePerm('invoices_print', 'طباعة الفواتير')) return;
    for (const id of ids) { await printInvoiceFromTable(id); await new Promise(r => setTimeout(r, 1200)); }
};
...
window.doLogout = async () => {
    ...
    if (currentUser) await setDoc(doc(db, "users", currentUser.uid), { forceLogout: false, currentSessionId: null, lastSeen: serverTimestamp() }, { merge: true });
    localStorage.removeItem('baron_session_id');
    if (maintenanceUnsub) maintenanceUnsub();
    await signOut(auth);
    window.location.href = "https://zadelrahmanzad-boop.github.io/baron1/";
...
window.connectQZ = async () => {
    ...
    try {
        ...
        printers.forEach(p => { const opt = document.createElement('option'); opt.value = p; opt.textContent = p; if (p === qzPrinter) opt.selected = true; sel.appendChild(opt); });
        sel.disabled = false;
...
window.savePrinterSettings = () => {
    ...
    if (sel.value) {
        ...
        qzPrinterCopies = parseInt(copies); qzPaperWidth = width;
        document.getElementById('printerStatusDot').style.background = '#27ae60';
